...
const getCount=(StudentRecords)=>reduce(StudentRecords,(a,b)=>({...a,...count(a,b)}),{pass:0,fail:0});

const processMarkSheet=()=> {
    const studentData=map(markSheets,(getStudentDetails));
    let rank=0;
    const finalMarkSheet=map(studentData,(markSheet,index,array)=> ({
       ...markSheet,
